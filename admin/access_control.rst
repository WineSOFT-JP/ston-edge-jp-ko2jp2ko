...
각 조건에 대해 개별적으로 응답 코드를 설정할 수도있다. ::
   ...
   # n 개의 조건을 결합 (AND)하기 위해서는 그리고를 사용한다.
...
-  **HEADER**
   ...
   $HEADER[Key : Value]로 표기한다. Value는 명확한 표현과 패턴을 인식한다. $ HEADER [Key :]와 같이 구분 기호가 Value가 빈 문자열이면 요청 헤더의 값이 비어있는 경우를 의미한다. $ HEADER [Key]와 같이 구분없이 Key 만 명시되어있는 경우 Key에 대응하는 헤더의 존재 유무를 조건으로 판단한다.
