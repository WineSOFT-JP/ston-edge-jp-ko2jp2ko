...
제한없이 클라이언트 요청을 허용하면 시스템에 과도한 부하가 발생할 수있다. 시스템 과부하는 실질적으로 장애이다. 적절한 수치로 클라이언트의 요청을 거부하여 시스템을 보호하기 ::
...
첫째, 클라이언트의 HTTP 요청이 어떻게 처리되는지를 이해한다. 캐시 처리의 결과는 Squid처럼TCP_ *로 명명 된 각 표현마다 캐시 서버가 처리 한 방식을 의미한다.
...
- SNMP의 cache.vhost.status 및 XML / JSON 통계 Host.State 값이が「Emergency」로 제공된다.
